...
namespace ego_planner
{
    ...
    void PlannerInterface::setCurvatureParams(double max_curvature, double weight)
    {
        bspline_optimizer_rebound_->setCurvatureParams(max_curvature, weight);
    }

    void PlannerInterface::setDoubleCircleFootprint(double radius, double front_offset, double rear_offset)
    {
        grid_map_->setInflateRadius(radius);
        grid_map_->setDoubleCircleOffsets(front_offset, rear_offset);
    }
    ...
    {
        cur_pose_ = cur_pose;
        bspline_optimizer_rebound_->setFallbackYaw(cur_pose.z);
    ...
    void PlannerInterface::makePlan()
    {
        ...
        local_target_vel[0] = cos(_global_plan_traj_[_global_plan_traj_.size()-1].theta);//根据实际需求修改接入
        local_target_vel[1] = sin(_global_plan_traj_[_global_plan_traj_.size()-1].theta);
