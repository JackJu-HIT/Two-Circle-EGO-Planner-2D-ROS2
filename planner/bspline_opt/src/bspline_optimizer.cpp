...
void BsplineOptimizer::setCurvatureParams(double max_curvature, double weight)
{
  // if (!std::isfinite(max_curvature) || max_curvature <= 0 ||
  //     !std::isfinite(weight) || weight < 0)
  //   throw std::invalid_argument("Curvature limit must be positive and weight nonnegative (finite)");
  // max_curvature_ = max_curvature;
  // lambda5_ = weight;
}

double BsplineOptimizer::estimateSegmentYaw(const Eigen::Vector2d& from,
                                             const Eigen::Vector2d& to) const
{
  const Eigen::Vector2d delta = to - from;
  return delta.squaredNorm() > 1e-12 ? std::atan2(delta.y(), delta.x()) : fallback_yaw_;
}
...
double BsplineOptimizer::estimateControlPointYaw(const Eigen::MatrixXd& q, int id) const
{
  ...
  // Widen the neighbourhood for repeated control points at a stopped endpoint.
  for (int width = 1; width <= last; ++width) {
    const Eigen::Vector2d a = q.col(std::max(0, id - width));
    const Eigen::Vector2d b = q.col(std::min(last, id + width));
    if ((b - a).squaredNorm() > 1e-12) return estimateSegmentYaw(a, b);
  }
  return fallback_yaw_;
}
...
std::vector<std::vector<Eigen::Vector2d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
{
    ...
    int i_end = static_cast<int>(init_points.cols()) - order_;
    for (int i = order_; i <= i_end; ++i)
    {
      const int samples = std::max(1, static_cast<int>(std::ceil(
          2.0 * (init_points.col(i) - init_points.col(i - 1)).norm() / grid_map_->getResolution())));
      for (int sample = 0; sample <= samples; ++sample)
      {
        const double a = 1.0 - static_cast<double>(sample) / samples;
        occ = grid_map_->getInflateOccupancy(a * init_points.col(i - 1) + (1 - a) * init_points.col(i), estimateSegmentYaw(init_points.col(i - 1), init_points.col(i)));
        // cout << setprecision(5);
    ...
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      ...
      for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
      {
        ...
        while (Astar_id >= 0 && Astar_id < (int)a_star_pathes[i].size())
        {
          ...
          if (Astar_id < 0 || Astar_id >= static_cast<int>(a_star_pathes[i].size())) break;
        ...
        if (found_intersection)
        {
          ...
          if (length > 1e-5)
          {
            ...
            {
              occ = grid_map_->getInflateOccupancy((a / length) * intersection_point + (1 - a / length) * cps_.points.col(j), estimateControlPointYaw(cps_.points, j));
        ...
        for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
        {
          ...
          if (found_intersection)
          {
            ...
            if (length > 1e-5)
            {
              for (double a = length; a >= 0.0; a -= grid_map_->getResolution())
              {
                bool occ = grid_map_->getInflateOccupancy((a / length) * intersection_point + (1 - a / length) * cps_.points.col(j), estimateControlPointYaw(cps_.points, j));
...
bool BsplineOptimizer::rebound_optimize()
{
  ...
  do
  {
        ...
        result == lbfgs::LBFGS_STOP)
    {
      ...
      UniformBspline traj(cps_.points, 3, bspline_interval_);
      double tm, tmp;
      traj.getTimeSpan(tm, tmp);
      double t_step = (tmp - tm) / ((traj.evaluateDeBoorT(tmp) - traj.evaluateDeBoorT(tm)).norm() / grid_map_->getResolution());
      // Follow SCAN: check the nearest 2/3 with heading from adjacent samples.
      for (double t = tm; t < tmp * 2 / 3; t += t_step)
      {
        Eigen::Vector2d pos = traj.evaluateDeBoorT(t);
        Eigen::Vector2d pos_next = traj.evaluateDeBoorT(std::min(t + t_step, tmp));
...
void BsplineOptimizer::combineCostRebound(const double *x, double *grad, double &f_combine, const int n)
{
  ...
  /* 计算各代价（均为 2D）*/
  double f_smoothness = 0, f_distance = 0, f_feasibility = 0, f_kappa = 0;
  ...
  if (lambda5_ > 0) calKappaCost(cps_.points, f_kappa, g_kappa);

  printf(" smoothness=%f distance=%f feasibility=%f curvature=%f\n",
         lambda1_ * f_smoothness, new_lambda2_ * f_distance,
         lambda3_ * f_feasibility, lambda5_ * f_kappa);


  /* 组合代价和梯度 */
  f_combine = lambda1_ * f_smoothness + new_lambda2_ * f_distance + lambda3_ * f_feasibility + lambda5_ * f_kappa;
  Eigen::MatrixXd grad_2D = lambda1_ * g_smoothness + new_lambda2_ * g_distance + lambda3_ * g_feasibility + lambda5_ * g_kappa;

  memcpy(grad, grad_2D.data() + 2 * order_, n * sizeof(grad[0]));
}

// Soft curvature bound on the actual uniform cubic B-spline, not its control polygon.
// Return an unweighted cost/gradient; lambda5_ is applied once in each objective.
void BsplineOptimizer::calKappaCost(const Eigen::MatrixXd &q, double &cost,
                                   Eigen::MatrixXd &gradient)
{
  cost = 0.0;
  gradient.setZero(2, q.cols());
  // Derivatives use normalized knot coordinate u, so curvature is independent
  // of bspline_interval_. Regularization is in spatial units (1 mm tangent).
  constexpr double tangent_epsilon_sq = 1e-6;
  constexpr int subdivisions = 4;
  for (int i = 0; i + 3 < q.cols(); ++i)
  {
    for (int sample = 0; sample <= subdivisions; ++sample)
    {
      const double u = static_cast<double>(sample) / subdivisions;
      const Eigen::Vector4d dB(-0.5 * (1-u) * (1-u), 1.5*u*u - 2*u,
                              -1.5*u*u + u + 0.5, 0.5*u*u);
      const Eigen::Vector4d ddB(1-u, 3*u-2, 1-3*u, u);
      const Eigen::Vector2d v = q.middleCols(i, 4) * dB;
      const Eigen::Vector2d a = q.middleCols(i, 4) * ddB;
      const double speed_sq = v.squaredNorm() + tangent_epsilon_sq;
      const double denom = speed_sq * std::sqrt(speed_sq);
      const double kappa = (v.x() * a.y() - v.y() * a.x()) / denom;
      const double violation = std::abs(kappa) - max_curvature_;
      if (violation <= 0) continue;

      // Trapezoidal weights avoid double-counting shared knot samples.
      const double weight = (sample == 0 || sample == subdivisions ? 0.5 : 1.0) / subdivisions;
      cost += weight * violation * violation;
      const double dc_dk = 2.0 * weight * violation * (kappa >= 0 ? 1.0 : -1.0);
      const Eigen::Vector2d dk_dv = Eigen::Vector2d(a.y(), -a.x()) / denom
                                    - 3.0 * kappa * v / speed_sq;
      const Eigen::Vector2d dk_da = Eigen::Vector2d(-v.y(), v.x()) / denom;
      for (int j = 0; j < 4; ++j)
        gradient.col(i + j) += dc_dk * (dB[j] * dk_dv + ddB[j] * dk_da);
    }
  }
}
...
void BsplineOptimizer::combineCostRefine(const double *x, double *grad, double &f_combine, const int n)
{
  memcpy(cps_.points.data() + 2 * order_, x, n * sizeof(x[0]));
  /* 计算各代价*/
  double f_smoothness = 0, f_fitness = 0, f_feasibility = 0, f_kappa = 0;
  ...
  Eigen::MatrixXd g_kappa = Eigen::MatrixXd::Zero(2, cps_.points.cols());
  if (lambda5_ > 0) calKappaCost(cps_.points, f_kappa, g_kappa);
  ...
  /* 组合代价和梯度 */
  f_combine = lambda1_ * f_smoothness + lambda4_ * f_fitness + lambda3_ * f_feasibility + lambda5_ * f_kappa;
  Eigen::MatrixXd grad_2D = lambda1_ * g_smoothness + lambda4_ * g_fitness + lambda3_ * g_feasibility + lambda5_ * g_kappa;
  memcpy(grad, grad_2D.data() + 2 * order_, n * sizeof(grad[0]));
}
