...
namespace ego_planner
{
  ...
  public:
    ...
    void setParam();
    void setCurvatureParams(double max_curvature, double weight);
    void setFallbackYaw(double yaw) { fallback_yaw_ = yaw; }
  ...
  private:
    friend struct CurvatureTestAccess;
    shared_ptr<GridMap2D> grid_map_;
    double fallback_yaw_ = 0.0;
    double estimateSegmentYaw(const Eigen::Vector2d& from, const Eigen::Vector2d& to) const;
    ...
    double lambda5_;               // curvature penalty weight
    double max_curvature_ = 2.0;   // 1/m; soft bound on absolute curvature
